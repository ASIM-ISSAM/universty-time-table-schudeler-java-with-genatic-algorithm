...
public class GenaticAlgorithm {
    ...
    Population crossoverPopulation(Population population) {
        ...
        IntStream.range(Driver.NUMB_OF_ELITE_SCHEDULES, population.getSchedules().size()).forEach(x -> {
            if (Driver.CROSSOVER_RATE > Math.random()) {
                Schedule schedule1 = selectToournamentPopulation(population).sortByFitnes().getSchedules().get(0);
                Schedule schedule2 = selectToournamentPopulation(population).sortByFitnes().getSchedules().get(0);
    ...
    Population selectToournamentPopulation(Population population) {
